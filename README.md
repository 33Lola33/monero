...
## Table of Contents
  ...
  - [Compiling MyNewCoin from source](#compiling-monero-from-source)
...
- It is HIGHLY recommended that you join the #monero-dev IRC channel if you are developing software that uses MyNewCoin. Due to the nature of this open source software project, joining this channel and idling is the best way to stay updated on best practices and new developments in the MyNewCoin ecosystem. All you need to do is join the IRC channel and idle to stay updated with the latest in MyNewCoin development. If you do not, you risk wasting resources on developing integrations that are not compatible with the MyNewCoin network. The MyNewCoin core team and community continuously make efforts to communicate updates, developments, and documentation via other platforms – but for the best information, you need to talk to other MyNewCoin developers, and they are on IRC. #monero-dev is about MyNewCoin development, not getting help about using MyNewCoin, or help about development of other software, including yours, unless it also pertains to MyNewCoin code itself. For these cases, checkout #monero.
...
The [MyNewCoin Research Lab](https://src.getmonero.org/resources/research-lab/) is an open forum where the community coordinates research into MyNewCoin cryptography, protocols, fungibility, analysis, and more. We welcome collaboration and contributions from outside researchers! Because not all Lab work and publications are distributed as traditional preprints or articles, they may be easy to miss if you are conducting literature reviews for your own MyNewCoin research. You are encouraged to get in touch with the MyNewCoin research community if you have questions, wish to collaborate, or would like guidance to help avoid unnecessarily duplicating earlier or known work.

The MyNewCoin research community is available on IRC in [#monero-research-lab on Libera](https://web.libera.chat/#monero-research-lab), which is also accessible via Matrix.

## Announcements

- You can subscribe to an [announcement listserv](https://lists.getmonero.org) to get critical announcements from the MyNewCoin core team. The announcement list can be very helpful for knowing when software updates are needed.
...
MyNewCoin is a private, secure, untraceable, decentralised digital currency. You are your bank, you control your funds, and nobody can trace your transfers unless you allow them to do so.

**Privacy:** MyNewCoin uses a cryptographically sound system to allow you to send and receive funds without your transactions being easily revealed on the blockchain (the ledger of transactions that everyone has). This ensures that your purchases, receipts, and all transfers remain private by default.

**Security:** Using the power of a distributed peer-to-peer consensus network, every transaction on the network is cryptographically secured. Individual wallets have a 25-word mnemonic seed that is only displayed once and can be written down to backup the wallet. Wallet files should be encrypted with a strong passphrase to ensure they are useless if ever stolen.

**Untraceability:** By taking advantage of ring signatures, a special property of a certain type of cryptography, MyNewCoin is able to ensure that transactions are not only untraceable but have an optional measure of ambiguity that ensures that transactions cannot easily be tied back to an individual user or computer.

**Decentralization:** The utility of MyNewCoin depends on its decentralised peer-to-peer consensus network - anyone should be able to run the monero software, validate the integrity of the blockchain, and participate in all aspects of the monero network using consumer-grade commodity hardware. Decentralization of the monero network is maintained by software development that minimizes the costs of running the monero software and inhibits the proliferation of specialized, non-commodity hardware.

## About this project

This is the core implementation of MyNewCoin. It is open source and completely free to use without restrictions, except for those specified in the license agreement below. There are no restrictions on anyone creating an alternative implementation of MyNewCoin that uses the protocol and network in a compatible manner.
...
**Anyone is welcome to contribute to MyNewCoin's codebase!** If you have a fix or code change, feel free to submit it as a pull request directly to the "master" branch. In cases where the change is relatively small or does not affect other parts of the codebase, it may be merged in immediately by any one of the collaborators. On the other hand, if the change is particularly large or complex, it is expected that it will be discussed at length either well in advance of the pull request being submitted, or even directly on the pull request.

## Supporting the project

MyNewCoin is a 100% community-sponsored endeavor. If you want to join our efforts, the easiest thing you can do is support the project financially. Both MyNewCoin and Bitcoin donations can be made to **donate.getmonero.org** if using a client that supports the [OpenAlias](https://openalias.org) standard. Alternatively, you can send MNC to the MyNewCoin donation address via the `donate` command (type `help` in the command-line wallet for details).

The MyNewCoin donation address is:  
...
MyNewCoin uses a scheduled software/network upgrade (hard fork) mechanism to implement new features into the MyNewCoin software and network. This means that users of MyNewCoin (end users and service providers) should run current versions and upgrade their software when new releases are available. Software upgrades occur when new features are developed and implemented in the codebase. Network upgrades occur in tandem with software upgrades that modify the consensus rules of the MyNewCoin network. The required software for network upgrades will be available prior to the scheduled network upgrade date. Please check the repository prior to this date for the proper MyNewCoin software version. Below is the historical schedule and the projected schedule for the next upgrade.
...
| Software upgrade block height  | Date       | Fork version      | Minimum MyNewCoin version | Recommended MyNewCoin version | Details                                                                            |
| ------------------------------ | -----------| ----------------- | ---------------------- | -------------------------- | ---------------------------------------------------------------------------------- |
...
## Compiling MyNewCoin from source
...
MyNewCoin uses the CMake build system and a top-level [Makefile](Makefile) that
...
* Change to the root of the source code directory, change to the most recent release branch, and build:
    ...
    *Note*: The instructions above will compile the most stable release of the
    MyNewCoin software. If you would like to use and test the most recent software,
...
* Run MyNewCoin with `monerod --detach`
...
* **Optional**: use ccache not to rebuild translation units, that haven't really changed. MyNewCoin's CMakeLists.txt file automatically handles it
...
* Install the dependencies for MyNewCoin from the 'Debian' column in the table above.
...
* Clone MyNewCoin and checkout the most recent release version:
...
If you are running MyNewCoin in a jail, you need to add `sysvsem="new"` to your jail configuration, otherwise lmdb will throw the error message: `Failed to open lmdb environment: Function not implemented`.

MyNewCoin is also available as a port or package as `monero-cli`.
...
Note: you may encounter the following error when compiling the latest version of MyNewCoin as a normal user:
...
Using `depends` might also be easier to compile MyNewCoin on Windows than using MSYS. Activate Windows Subsystem for Linux (WSL) with a distro (for example Ubuntu), install the apt build-essentials and follow the `depends` steps as depicted above.
...
If you have an issue with building MyNewCoin with Trezor support, you can disable it by setting `USE_DEVICE_TREZOR=OFF`, e.g., 
...
## Installing MyNewCoin from a package
...
* Gentoo [MyNewCoin overlay](https://github.com/gentoo-monero/gentoo-monero)
...
While MyNewCoin isn't made to integrate with Tor, it can be used wrapped with torsocks, by
...
already, and runs Tor and MyNewCoin with the right configuration.
...
As of April 2022, the full MyNewCoin blockchain file is about 130 GB. One can store a pruned blockchain, which is about 45 GB.
...
For more detailed information see the ['Pruning' entry in the MyNewCoinpedia](https://www.getmonero.org/resources/moneropedia/pruning.html)

## Debugging

This section contains general instructions for debugging failed installs or problems encountered with MyNewCoin. First, ensure you are running the latest version built from the GitHub repo.
...
Configure MyNewCoin with the -D SANITIZE=ON cmake flag, eg:
