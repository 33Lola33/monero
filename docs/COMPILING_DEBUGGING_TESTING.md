...
The intended audience are developers, who want to leverage newly added tricks to MyNewCoin via `CMake`. The document will lower the entry point for these developers.
...
MyNewCoin can be compiled via the main `Makefile`, using one of several targets listed there.
...
It is possible to extract these `CMake` calls and modify them for your specific needs. For example, a minimal external cmake command to compile MyNewCoin, executed from within a newly created build directory could look like:

`cmake -S "$DIR_SRC" -DCMAKE_BUILD_TYPE=Release && make`

where the variable `DIR_SRC` is expected to store the path to the MyNewCoin source code.
