...
used by MyNewCoin. The protocol is largely inherited from cryptonote, but has
undergone some changes.

This document also may differ from the `struct bucket_head2` in MyNewCoin's
...
> if interested in learning about MyNewCoin p2p traffic!


## Header
This header is sent for every MyNewCoin p2p message.
...
MyNewCoin when the link is not encrypted. SSL has been proposed as a means to
...
### Command
An unsigned 32-bit little endian integer representing the MyNewCoin specific
...
### Requests
Requests are the basis of the admin protocol for MyNewCoin. The `Q` bit must be
