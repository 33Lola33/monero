# The Current/Future Status of ZMQ in MyNewCoin

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. MyNewCoin
...
matching supported by both MyNewCoin and ZMQ. The `format`, `context` and `event`
...
The MyNewCoin daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The MyNewCoin daemon also ensures that
...
The MyNewCoin daemon will send a `txpool_add` pub exactly once for each
