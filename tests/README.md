...
Core tests take longer than any other MyNewCoin tests, due to the high amount of computational work involved in validating core components.
...
To run only MyNewCoin's core tests (after building):
...
To run only MyNewCoin's crypto tests (after building):
...
To run only MyNewCoin's hash tests (after building):
...
To run only MyNewCoin's performance tests (after building):
...
To run only MyNewCoin's unit tests (after building):
